...
def build_steps(args=None):
    return [
        ...
        # Выкуп WB по когорте дня заказа (WB-10 §2, слово владельца 09-25): продажи отчёта по orderDt против заказов воронки,
        # окно 60 дней (25 зрелых на прогноз + 30 незрелых + запас), свои таблицы wb_buyout_cohort_*; читает только алерт
        # (строка «Выкуп по когорте»). Нефатален по правилу FATAL_STEPS: витрины его не читают, ночь доберёт.
        ("WB: выкуп по когорте", "python3 scripts/wb_buyout_cohort_step.py --days-back 60 --apply --approve-wb-cohort-write"),
        # Реклама WB: adv/v1/upd за 31 день до вчера, одно обращение, wb_ad_spend_daily (WB-6 §3а, слово владельца 09-23);
        # нефатален по правилу FATAL_STEPS: витрины и алерт таблицу не читают, окно 31 день доберёт следующей ночью.
        ("WB: реклама", "python3 loaders/wb_ads_loader.py"),
