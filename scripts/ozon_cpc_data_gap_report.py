...
ВАЖНО: «завершено N выгрузок → записано M строк» — НЕКОРРЕКТНОЕ сопоставление,
и пользоваться им как признаком успеха нельзя.

    marketplace_expenses ключуется по (expense_date, marketplace_code,
    marketplace_sku, expense_type) — без campaign_id.

Несколько кампаний на одну SKU схлопываются в одну строку расхода. Проверено на
2026-07-13: прогон 2026-08-31 записал 112 строк атрибуции по 112 РАЗНЫМ кампаниям,
но они покрывают лишь 83 SKU; по всей дате 147 строк атрибуции и 147 кампаний
дают 87 SKU, а с ненулевым расходом — 110 строк и 73 SKU. Отсюда и разница
«112 записал / 70 видит детектор»: это не потеря, а разный уровень агрегации.

Следствие: отношение units к строкам ничего не измеряет. Осмысленны только
абсолютный ноль (записали 0 строк при завершённых выгрузках — см. verify_write_result
в лоадере) и сравнение с соседними днями, которым занят этот скрипт.
