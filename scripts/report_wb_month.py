...
строки из файлов сырья бэкфилла — --rows-from-files, пока таблицы нет). Дата
реализации = rrDate («Дата операции»). Карта колонок и что проверено по значениям
— docs/wb_report_model.md.
...
КОЛОНКИ (по дню, суммы за день; знак: Продажа +, Возврат −):
    ...
    Комиссия (факт)     Оборот − Σ forPay± — у владельца вместо неё константа 42 % от оборота
    Комиссия по образцу 0,42 × Оборот — справочно и для приёмки
    ...
    Выручка по образцу  (Оборот − 0,42 × Оборот) / НДС − НДС за возмещение — его E, до копейки
    ...
    Логистика           Σ deliveryService / НДС — близко к его, не тождество (docs/wb_report_model.md)
    Реклама             advert-api /adv/v1/upd за месяц (фактические списания), 1 обращение; --no-fetch — пусто
    Эквайринг           Σ acquiringFee / НДС
    Прочее              (Σ paidStorage + Σ penalty) / НДС
...
YOUNG_DAYS = 2
MANUAL_COMMISSION_RATE = Decimal("0.42")   # константа владельца: D = C / B = 0,42 на всех днях листа
...
# Расхождения с листом, которые владелец назвал своей стороной (WB-5 §3): печатаем, не чиним, в отказ не считаем.
KNOWN_MANUAL_DIFFS = {("acquiring", "2026-09-03"), ("acquiring", "2026-09-07"), ("other", "2026-09-02")}
MONEY_FIELDS = ("retail_price_with_disc", "retail_amount", "for_pay", "ppvz_reward", "rebill_logistic_cost",
                "delivery_service", "acquiring_fee", "paid_storage", "penalty", "cashback_discount")
SELECT = ("rrd_id,rr_date,seller_oper_name,doc_type,vendor_code,tech_size,nm_id,quantity," + ",".join(MONEY_FIELDS))
...
def load_rows_db(sb, d1, d2):
    return stale_keys.read_window_rows(sb, loader.TABLE, SELECT, [("gte", "rr_date", d1), ("lte", "rr_date", d2)], ["rr_date", "rrd_id"])
...
def build_daily(rows, days, cost_fn, ads_by_day, today, ads_known=True):
    ...
    for r in rows:
        ...
        if op in ("Продажа", "Возврат"):
            ...
            s["turnover"] += sign * D(r["retail_price_with_disc"])
        ...
        s["delivery"] += D(r["delivery_service"]); s["acquiring_raw"] += D(r["acquiring_fee"])
    ...
    for d in days:
        ...
        commission_manual = s["turnover"] * MANUAL_COMMISSION_RATE
        ...
        logistics, acquiring, other = s["delivery"] / vat, s["acquiring_raw"] / vat, (s["storage"] + s["penalty"]) / vat
        margin = revenue - cogs
        ...
        row = {"date": d, "vat": vat, "rows": c["rows"], "positions": c["positions"], "no_cost_positions": c["no_cost_positions"],
               ...
               "reward": s["reward"], "rebill": s["rebill"], "storage": s["storage"], "penalty": s["penalty"]}
...
def total_row(rows):
    ...
    for k in ("rows", "positions", "no_cost_positions"):
        t[k] = sum(r[k] for r in rows)
    for k in ("turnover", "commission", "commission_manual", "vat_refund", "revenue", "revenue_manual", "cogs", "margin", "logistics", "acquiring", "other", "fin_result", "reward", "rebill", "storage", "penalty"):
...
COLS = [("Дата реализации", "date", None), ("Оборот (с НДС), руб.", "turnover", "money"), ("Комиссия факт (с НДС), руб.", "commission", "money"),
        ("Комиссия факт, %", "commission_pct", "pct"), ("справочно: Комиссия по образцу 42 %", "commission_manual", "money"),
        ("Выручка, руб.", "revenue", "money"), ("справочно: Выручка по образцу (42 %)", "revenue_manual", "money"),
        ...
        ("Возмещение перемещения (с НДС)", "rebill", "money"), ("Хранение (с НДС)", "storage", "money"), ("Штрафы (с НДС)", "penalty", "money"),
...
CHECK_LINES = (
    ...
    ("Выручка по образцу (42 %, минус НДС за возмещение)", "revenue_manual", "revenue", True, ""),
    ("Эквайринг (acquiringFee / НДС)", "acquiring", "acquiring", True, "09-03 и 09-07 у владельца меньше отчёта — его сторона (WB-5 §3)"),
    ("Прочее (хранение + штрафы) / НДС", "other", "other", True, "09-02 у владельца +563,20 сверх отчёта — его сторона (WB-5 §3)"),
    ("Комиссия по образцу 42 %", "commission_manual", "commission", True, ""),
    ("Комиссия факт (оборот − forPay) против его 42 %", "commission", "commission", False, "у него константа 42 %, факт по отчёту 44,4 % — решение 4 WB-5: наш лист по факту"),
    ("Выручка факт против его выручки", "revenue", "revenue", False, "разница — комиссия факт против 42 %"),
    ("Логистика (deliveryService / НДС)", "logistics", "logistics", False, "правило его отнесения не найдено; расхождение печатается (решение 4 WB-5)"),
    ...
    ("Фин. рез. (наш факт против его)", "fin_result", "fin_result", False, "комиссия факт против 42 % + СС + логистика"),
)
...
def main(argv=None):
    ...
    print(f"строк отчёта за {d1} … {days[-1]}: {len(rows)} (" + ("файлы " + args.rows_from_files if args.rows_from_files else loader.TABLE) + ")")
    ...
    daily = build_daily(rows, days, cost_fn, ads_by_day, today, ads_known)
    total = total_row(daily)
    young = [r["date"] for r in daily if r["young"]]
    notes = [f"Источник: отчёт реализации WB (finance-api sales-reports/detailed, period=daily), дата — rrDate. Строк {len(rows)}.",
             f"Комиссия факт = Оборот − forPay; у владельца константа 42 % — она в справочных колонках и в приёмке. НДС {total['vat']}.",
             ...
             ads_note, "Логистика = deliveryService / НДС — с листом владельца близко, не тождество (docs/wb_report_model.md).",
    ...
    print(f"по образцу (42 %): комиссия {total['commission_manual']:,.2f}, выручка {total['revenue_manual']:,.2f}; НДС за возмещение {total['vat_refund']:,.2f}")
